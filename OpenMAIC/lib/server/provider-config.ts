...
export function resolveVideoApiKey(providerId: string, clientKey?: string): string {
  if (clientKey) return clientKey;
  const cfg = getConfig();
  if (cfg.video[providerId]?.apiKey) return cfg.video[providerId].apiKey;
  if (providerId === 'seedance') {
    return (
      cfg.image.seedream?.apiKey ||
      process.env.VIDEO_SEEDANCE_API_KEY ||
...
export function resolveVideoBaseUrl(
  ...
  if (clientBaseUrl) return clientBaseUrl;
  const cfg = getConfig();
  if (cfg.video[providerId]?.baseUrl) return cfg.video[providerId].baseUrl;
  if (providerId === 'seedance') {
    return (
      cfg.image.seedream?.baseUrl ||
      process.env.VIDEO_SEEDANCE_BASE_URL ||
      process.env.IMAGE_SEEDREAM_BASE_URL
    );
  }
  return undefined;
}
