...
async function findExistingMediaFile(
  mediaDir: string,
  elementId: string,
  extensions: string[],
  ...
  for (const ext of extensions) {
    ...
    try {
      const stat = await fs.stat(path.join(mediaDir, filename));
      if (stat.isFile() && stat.size > 0) return filename;
    } catch {
...
export async function generateMediaForClassroom(
  ...
  // Collect media generation requests from outlines and enforce the course budget.
  const requests = outlines
    .flatMap((o) => o.mediaGenerations ?? [])
    .filter((req, index, arr) => arr.findIndex((item) => item.elementId === req.elementId) === index);
  ...
  const imageRequests = requests
    .filter((r) => r.type === 'image' && imageProviderIds.length > 0)
    .slice(0, MAX_AI_IMAGES_PER_COURSE);
  const videoRequests = requests
    .filter((r) => r.type === 'video' && videoProviderIds.length > 0)
    .slice(0, MAX_AI_VIDEOS_PER_COURSE);

  const generateImages = async () => {
    for (const req of imageRequests) {
      try {
        const existing = await findExistingMediaFile(mediaDir, req.elementId, [
          'png',
          'jpg',
          'jpeg',
          'webp',
        ...
        if (existing) {
          mediaMap[req.elementId] = mediaServingUrl(baseUrl, classroomId, `media/${existing}`);
          log.info(`Reusing existing image: ${existing}`);
          continue;
        }
  ...
  const generateVideos = async () => {
    for (const req of videoRequests) {
      try {
        const existing = await findExistingMediaFile(mediaDir, req.elementId, ['mp4', 'webm']);
        if (existing) {
          mediaMap[req.elementId] = mediaServingUrl(baseUrl, classroomId, `media/${existing}`);
          log.info(`Reusing existing video: ${existing}`);
          continue;
        }
