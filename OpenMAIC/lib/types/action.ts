...
/** Mocap — select a UE teacher motion clip/state for a speech segment */
export interface MocapAction extends ActionBase {
  type: 'mocap';
  mocapId: string;
  mocapAction?: string;
  mocapCoarse?: string;
...
/** Slide write text — progressively reveal text inside a PPT text element */
export interface SlideWriteTextAction extends ActionBase {
  type: 'slide_write_text';
  elementId: string;
  content: string;
  fontSize?: number;
  color?: string;
