...
import type { Action, SpeechAction, DiscussionAction, MocapAction } from '@/lib/types/action';
import type {
  EngineMode,
  TopicState,
...
function isBoardWritingMocap(mocapId: string | null): boolean {
...
export class PlaybackEngine {
  ...
  private currentMocapId: string | null = null;
  ...
  private async processNext(): Promise<void> {
    ...
    switch (action.type) {
      case 'speech': {
        ...
        const readingMs = estimateReadingMs();
        if (
          speechAction.writeElementId &&
          speechAction.writeContent &&
          isBoardWritingMocap(this.currentMocapId)
        ) {
          this.actionEngine.revealSlideText(
            speechAction.writeElementId,
            speechAction.writeContent,
            speechAction.writeColor,
          );
        }
      ...
      case 'mocap': {
        const mocapAction = action as MocapAction;
        this.currentMocapId = mocapAction.mocapId;
        this.callbacks.onMocapAction?.({
          mocapId: mocapAction.mocapId,
          mocapAction: mocapAction.mocapAction,
          mocapCoarse: mocapAction.mocapCoarse,
          confidenceScore: mocapAction.confidenceScore,
          sourceText: mocapAction.sourceText,
          teacherStatus: mocapAction.teacherStatus,
          ueAction: mocapAction.ueAction,
        });
        this.processNext();
        break;
      }
