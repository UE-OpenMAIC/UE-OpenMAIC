...
async function generateSlideContent(
  ...
  if (isVideoOnlySlide(outline)) {
    processedElements = buildFullBleedVideoElements(outline, processedElements, canvasWidth, canvasHeight);
  }
  ...
  } else if (generatedData.background) {
...
async function generateQuizContent(
  ...
  return generateBlackboardSceneContent(outline);
}

function generateBlackboardSceneContent(outline: SceneOutline): GeneratedSlideContent {
  const blackboardImage = '/images/blackboard-custom.png';

  const titleText = sanitizeBlackboardDisplayTitle(outline.title || '黑板讲解');
  const boardTextTop = 170;
  ...
  const elements: PPTElement[] = [
    {
      ...
      rotate: 0,
      lock: false,
      content: `<p style="font-size:40px;"><strong>${escapeHtml(titleText)}</strong></p>`,
      ...
      textType: 'title',
      fill: 'rgba(0,0,0,0)',
      outline: { width: 0, color: '#000000', style: 'solid' },
      shadow: { h: 0, v: 0, blur: 0, color: '#000000', angle: 0, opacity: 0 },
    } as PPTElement,
    {
      id: `el_${nanoid(8)}`,
      type: 'image',
      ...
      src: blackboardImage,
      opacity: 1,
      shadow: { h: 0, v: 8, blur: 18, color: '#000000', angle: 0, opacity: 0.2 },
      borderRadius: 16,
      outline: { width: 0, color: '#000000', style: 'solid' },
    } as PPTElement,
    {
  ...
  return {
    elements,
    background: { type: 'solid', color: '#0b1320' },
    remark: outline.description,
  };
}

function isVideoOnlySlide(outline: SceneOutline): boolean {
  return outline.type === 'slide' && !!outline.mediaGenerations?.some((media) => media.type === 'video');
}

function buildFullBleedVideoElements(
  outline: SceneOutline,
  elements: PPTElement[],
  canvasWidth: number,
  canvasHeight: number,
): PPTElement[] {
  const firstVideoRequest = outline.mediaGenerations?.find((media) => media.type === 'video');
  const existingVideo = elements.find(
    (element): element is Extract<PPTElement, { type: 'video' }> => element.type === 'video',
  );
  const src = existingVideo?.src || firstVideoRequest?.elementId || 'gen_vid_1';

  return [
    {
      id: existingVideo?.id || `video_${nanoid(8)}`,
      type: 'video',
      left: 0,
      top: 0,
      width: canvasWidth,
      ...
      src,
      autoplay: false,
      poster: existingVideo?.poster,
      ext: existingVideo?.ext,
    },
  ];
}

function sanitizeBlackboardDisplayTitle(title: string): string {
  const cleaned = (title || '')
    .replace(/逐句书写/gi, '')
    .replace(/逐行书写/gi, '')
    .replace(/板书推导/gi, '')
...
function buildBlackboardLines(outline: SceneOutline): string[] {
  const limitLine = (line: string) => {
  ...
  const sanitize = (line: string) =>
    ASSESSMENT_TERMS.reduce((acc, term) => acc.replace(new RegExp(term, 'gi'), ''), line)
      .replace(/\s+/g, ' ')
      .replace(/[：:，,、。；;]+$/g, '')
      .trim();
  ...
  const fromKeyPoints = (outline.keyPoints || []).flatMap((line) => splitToLines(line));
  if (fromKeyPoints.length > 0) return fromKeyPoints.map(limitLine).slice(0, 4);

  const base = splitToLines(
    `${sanitize(outline.description || '')} ${sanitize(outline.title || '')}`,
  )
    .map(limitLine)
    .slice(0, 4);
...
export async function generateSceneActions(
  ...
  if (outline.type === 'slide' && 'elements' in content) {
    if (shouldUseBoardWritingModule(outline, content.elements)) {
      return generateBoardWritingSlideActions(outline, content.elements);
    }

    if (isVideoOnlySlide(outline)) {
      return generateVideoSlideActions(outline, content.elements);
    }
  ...
  if (outline.type === 'quiz' && 'elements' in content) {
    return generateBlackboardQuizActions(outline, content.elements);
  }
...
function generateVideoSlideActions(outline: SceneOutline, elements: PPTElement[]): Action[] {
  const video = elements.find((element) => element.type === 'video');
  if (!video) return generateDefaultSlideActions(outline, elements);

  const isIntroVideo = /导读|引入|开场|背景|intro|opening|overview|background/i.test(
    `${outline.title} ${outline.description}`,
  );
  const transition =
    outline.language === 'en-US'
      ? isIntroVideo
        ? 'Let us begin with a short guide video to enter today\'s lesson context.'
        : 'Before the final summary, let us watch this short clip and connect the visuals back to what we have just learned.'
      : isIntroVideo
        ? '我们先看一段导读短片，进入今天这节课的情境。'
        : '在最后总结之前，我们先看一段短片，把刚才学到的内容用画面串起来。';

  return [
    {
      id: `action_${nanoid(8)}`,
      type: 'speech',
      title: outline.language === 'en-US' ? 'Transition' : '串场词',
      text: transition,
    },
    {
      id: `action_${nanoid(8)}`,
      type: 'play_video',
      title: outline.language === 'en-US' ? 'Play clip' : '播放短片',
      elementId: video.id,
    },
  ];
}

function shouldUseBoardWritingModule(outline: SceneOutline, elements: PPTElement[]): boolean {
  const text = [
    outline.title,
    outline.description,
    ...(outline.keyPoints || []),
    ...elements
      ...
      .map((element) => ('content' in element ? stripHtmlText(element.content) : '')),
  ]
    .filter(Boolean)
    .join('\n');

  return BOARD_WRITING_KEYWORDS.some((keyword) => text.includes(keyword));
}

function generateBoardWritingSlideActions(outline: SceneOutline, elements: PPTElement[]): Action[] {
  const textElements = elements
    .filter((element) => element.type === 'text')
    .filter((element) => stripHtmlText(element.content).length > 0)
    .sort((a, b) => a.top - b.top);

  const writableElements = textElements.filter(
    (element) => element.top > 120 && element.textType !== 'title',
  );
  const targets = writableElements.length > 0 ? writableElements : textElements.slice(1);

  if (targets.length === 0) {
    return generateDefaultSlideActions(outline, elements);
  }

  const actions: Action[] = [];

  targets.slice(0, 6).forEach((element, index) => {
    const lines = extractTextLines(element.content);
    ...
    lines.forEach((line, lineIndex) => {
      ...
      actions.push({
        id: `action_${nanoid(8)}`,
        type: 'speech',
        title: index === 0 && lineIndex === 0 ? '板书讲解' : '继续板书',
        text: line,
        mocapSelectorText: `板书书写：${line}`,
        writeElementId: element.id,
        writeContent: makeLinesHtml(visibleLines, extractFontSize(element.content) ?? 22),
        writeFontSize: extractFontSize(element.content) ?? 22,
        writeColor: element.defaultColor,
      });
    });
  });
...
function generateBlackboardQuizActions(outline: SceneOutline, elements: PPTElement[]): Action[] {
  const target = elements.find(
    (element): element is Extract<PPTElement, { type: 'text' }> =>
      element.type === 'text' && element.id === 'blackboard_text',
  );
  if (!target) {
    return generateBoardWritingSlideActions(outline, elements);
  }

  const lines = buildBlackboardLines(outline);
  if (lines.length === 0) {
    return generateDefaultSlideActions(outline, elements);
  }

  const fontSize = lines.length > 3 ? 30 : 34;

  return lines.map((line, index) => ({
    id: `action_${nanoid(8)}`,
    type: 'speech',
    title: index === 0 ? '板书讲解' : '继续板书',
    text: line,
    mocapSelectorText: `板书书写：${line}`,
    writeElementId: target.id,
...
export function createSceneWithActions(
  ...
  if (outline.type === 'quiz' && 'elements' in content) {
    const defaultTheme: SlideTheme = {
      ...
      themeColors: ['#58a36d', '#f4c95d', '#5ba8ff', '#f58a5c', '#9dd8c8'],
      fontColor: '#ecf4ed',
      fontName: 'Microsoft YaHei',
      outline: { color: '#d4b278', width: 1, style: 'solid' },
      shadow: { h: 0, v: 0, blur: 6, color: '#000000' },
    };
