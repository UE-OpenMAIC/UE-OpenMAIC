...
- If this slide contains a generated video, the video should be the only meaningful visual element and should fill the whole PPT canvas: `left: 0`, `top: 0`, `width: {{canvas_width}}`, `height: {{canvas_height}}`.
- Do NOT leave space for a title, caption, or other slide text on generated-video slides. The transition line belongs in speech actions before `play_video`, not on the slide.
- Set `autoplay` to `false`; playback is controlled by a `play_video` action.
