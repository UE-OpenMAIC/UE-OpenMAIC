...
function resolveSeedanceModel(model: string | undefined, generateAudio?: boolean): string {
  const selected = model || DEFAULT_MODEL;
  if (!generateAudio) return selected;

  // Seedance 1.0 / 1.0 Fast commonly returns silent t2v output. For required
  // audio, prefer the 1.5 Pro audio-capable endpoint unless the caller has
  // explicitly chosen a 2.x audio-capable model.
  if (selected.includes('seedance-2-')) return selected;
  if (selected === 'doubao-seedance-1-5-pro-251215') return selected;
...
export async function submitSeedanceTask(
  ...
  const body: Record<string, unknown> = {
    model: resolveSeedanceModel(config.model, generateAudio),
    content: [
      {
        type: 'text',
        text: generateAudio
          ? `${options.prompt}\nGenerate a synchronized natural audio track with ambient sound and motion sound effects.`
          : options.prompt,
      },
    ],
