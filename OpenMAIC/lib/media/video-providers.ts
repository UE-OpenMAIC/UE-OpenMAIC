...
export const VIDEO_PROVIDERS: Record<VideoProviderId, VideoProviderConfig> = {
  seedance: {
    ...
    models: [
      { id: 'doubao-seedance-1-5-pro-251215', name: 'Seedance 1.5 Pro (Audio)' },
      {
...
export function normalizeVideoOptions(
  ...
  // Duration: use the closest supported value not exceeding the request.
  if (provider.supportedDurations && provider.supportedDurations.length > 0) {
    if (!normalized.duration || !provider.supportedDurations.includes(normalized.duration)) {
      const sorted = [...provider.supportedDurations].sort((a, b) => a - b);
      normalized.duration =
        sorted.filter((duration) => duration <= (normalized.duration || sorted[0])).pop() ||
        sorted[0];
    }
  }
