...
export function useSceneGenerator(options: UseSceneGeneratorOptions = {}) {
  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      const state = store.getState();
      const outline =
        state.failedOutlines.find((o) => o.id === outlineId) ||
        state.outlines.find((o) => o.id === outlineId);
      if (!outline || !state.stage) return;

      const existingScene = state.scenes.find((scene) => scene.order === outline.order);
      const wasCurrentScene = existingScene?.id === state.currentSceneId;
      ...
      try {
        ...
        const latestScenes = store.getState().scenes;
        const sceneExists = latestScenes.some((scene) => scene.order === outline.order);
        if (sceneExists) {
          const nextScenes = latestScenes
            .map((scene) => (scene.order === outline.order ? actionsResult.scene! : scene))
            .sort((a, b) => a.order - b.order);
          store.getState().setScenes(nextScenes);
          if (wasCurrentScene) {
            store.getState().setCurrentSceneId(actionsResult.scene.id);
          }
        } else {
