...
const getDefaultAudioConfig = () => ({
  ttsProviderId: 'doubao-tts' as TTSProviderId,
...
function ensureValidProviderSelections(state: Partial<SettingsState>): void {
  ...
  if (
    ...
    state.imageModelId === 'doubao-seedream-5-0-260128'
  ) {
    state.imageModelId = defaultImageConfig.imageModelId;
  }
          ...
          try {
            ...
            set((state) => {
              ...
              const currentImageModelId =
                validImageProvider === 'seedream' &&
                state.imageModelId === 'doubao-seedream-5-0-260128'
                  ? 'doubao-seedream-4-5-251128'
                  : state.imageModelId;
              const validImageModel = validImageProvider
                ? recoveredImageModel ||
                  validateModel(currentImageModelId, imageModels) ||
              ...
              const currentVideoModelId =
                validVideoProvider === 'seedance' &&
                (state.videoModelId === 'doubao-seedance-1-0-pro-fast-251015' ||
                  state.videoModelId === 'doubao-seedance-1-0-pro-250528')
                  ? 'doubao-seedance-1-5-pro-251215'
                  : state.videoModelId;
              const validVideoModel = validVideoProvider
                ? recoveredVideoModel ||
                  validateModel(currentVideoModelId, videoModels) ||
                  videoModels[0]?.id ||
                  ''
                : '';
