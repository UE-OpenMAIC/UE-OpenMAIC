...
export const useMediaGenerationStore = create<MediaGenerationState>()((set, get) => ({
  ...
  enqueueTasks: (stageId, requests) => {
    ...
    const updatedTasks: Record<string, MediaTask> = {};
    for (const req of requests) {
      const existing = get().tasks[req.elementId];
      if (
      ...
      if (existing) {
        updatedTasks[req.elementId] = nextTask;
      } else {
        newTasks[req.elementId] = nextTask;
      }
    }
    if (Object.keys(newTasks).length > 0 || Object.keys(updatedTasks).length > 0) {
      set((s) => ({ tasks: { ...s.tasks, ...newTasks, ...updatedTasks } }));
    }
  },
