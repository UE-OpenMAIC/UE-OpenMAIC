...
function makeTextHtml(text: string, fontSize: number): string {
  const lines = escapeHtml(text)
    .split('\n')
    .map((line) => line || '&nbsp;');
  return lines.map((line) => `<p style="font-size:${fontSize}px;">${line}</p>`).join('');
}
...
export type SlideWriteController = {
  pause: () => void;
  resume: () => void;
  stop: () => void;
  finish: () => void;
...
function createFinishedSlideWriteController(): SlideWriteController {
  return {
    pause: () => {},
    resume: () => {},
    stop: () => {},
...
export class ActionEngine {
  ...
  async execute(action: Action): Promise<void> {
    ...
    switch (action.type) {
      ...
      case 'slide_write_text':
        return this.executeSlideWriteText(action as SlideWriteTextAction);
  ...
  private async executeSlideWriteText(action: SlideWriteTextAction): Promise<void> {
    this.revealSlideText(action.elementId, action.content, action.color);
  }

  public revealSlideText(elementId: string, content: string, color?: string): void {
    const sceneId = this.stageStore.getState().currentSceneId;
    if (!sceneId) return;

    const elementResult = this.stageAPI.element.get(sceneId, elementId);
    if (!elementResult.success || !elementResult.data || elementResult.data.type !== 'text') {
      log.warn(`slide text reveal target not found or not text: ${elementId}`);
      return;
    }

    this.stageAPI.element.update(sceneId, elementId, {
      content,
  ...
  public startSlideWriteText(
    action: SlideWriteTextAction,
    durationMs: number,
  ): SlideWriteController {
    const sceneId = this.stageStore.getState().currentSceneId;
    if (!sceneId) {
      return createFinishedSlideWriteController();
    }

    const elementResult = this.stageAPI.element.get(sceneId, action.elementId);
    if (!elementResult.success || !elementResult.data || elementResult.data.type !== 'text') {
      log.warn(`slide_write_text target not found or not text: ${action.elementId}`);
      return createFinishedSlideWriteController();
    }

    const text = stripHtml(action.content).trim();
    if (!text) {
      return createFinishedSlideWriteController();
    }

    const fontSize = action.fontSize ?? extractFontSize(action.content) ?? 22;
    const color = action.color ?? elementResult.data.defaultColor ?? '#333333';
    const safeDurationMs = Math.max(durationMs, 300);
    const stepMs = Math.max(24, Math.floor(safeDurationMs / Math.max(text.length, 1)));

    this.stageAPI.element.update(sceneId, action.elementId, {
      content: makeTextHtml('', fontSize),
      defaultColor: color,
      opacity: 1,
    });
    ...
    const tick = () => {
      ...
      index += 1;
      this.stageAPI.element.update(sceneId, action.elementId, {
        content: makeTextHtml(text.slice(0, index), fontSize),
      });
    ...
    return {
      ...
      resume: () => {
        if (stopped || !paused || index >= text.length) return;
        paused = false;
        timer = setTimeout(tick, stepMs);
      },
      stop: () => {
      ...
      finish: () => {
        ...
        this.stageAPI.element.update(sceneId, action.elementId, {
          content: makeTextHtml(text, fontSize),
        });
        resolveFinished();
      },
      finished,
