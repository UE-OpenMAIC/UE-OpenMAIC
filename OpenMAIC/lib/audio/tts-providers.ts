...
async function generateDoubaoTTS(
  ...
  const isLegacyV1Voice = /^BV\d+_/i.test(config.voice || '');
  const normalizedV3Voice = isLegacyV1Voice ? 'zh_female_vv_uranus_bigtts' : config.voice;

  const callV1 = async (): Promise<TTSGenerationResult> => {
    const v1Resp = await fetch('https://openspeech.bytedance.com/api/v1/tts', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        ...
        audio: {
          ...
          encoding: 'mp3',
          speed_ratio: Number((config.speed || 1.0).toFixed(2)),
          loudness_ratio: 1.0,
          rate: 24000,
        },
        request: {
          reqid: `openmaic_${Date.now()}`,
          text,
          text_type: 'plain',
          operation: 'query',
          with_timestamp: 1,
    ...
    if (!v1Resp.ok) {
      const v1Err = await v1Resp.text().catch(() => v1Resp.statusText);
      throw new Error(`Doubao TTS V1 failed (${v1Resp.status}): ${v1Err}`);
    }

    const v1Json = (await v1Resp.json()) as { data?: string; code?: number; message?: string };
    if (v1Json?.code !== 3000 || !v1Json?.data) {
      throw new Error(
        `Doubao TTS V1 returned no audio data: code=${v1Json?.code ?? 'unknown'} message=${v1Json?.message ?? ''}`,
      );
    }

    return { audio: new Uint8Array(Buffer.from(v1Json.data, 'base64')), format: 'mp3' };
  };

  // Legacy voices like BV700_V2_streaming are from the old stack.
  // For Doubao 2.0, map them to a known available 2.0 voice to avoid 55000000.
  ...
  if (!response.ok) {
    ...
    // Fallback to V1 when V3 grant/key doesn't match
    try {
      return await callV1();
    } catch (fallbackErr) {
      throw new Error(
        `Doubao TTS V3 failed (${response.status}): ${errorText}; V1 fallback failed: ${
    ...
    if (responseText[i] === '{') {
      ...
      if (depth === 0 && start >= 0) {
        ...
        if (chunk.code === 0 && chunk.data) {
          ...
          if (chunk.code === 55000000) {
            // speaker/resource mismatch: try V1 for non-legacy voices only.
            if (!isLegacyV1Voice) {
              return await callV1();
            }
            throw new Error(
              `Doubao TTS voice not compatible with current resource. Requested="${config.voice}", mapped="${normalizedV3Voice}"`,
            );
          }
