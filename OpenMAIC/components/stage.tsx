...
import type { EngineMode, TriggerEvent, Effect, MocapEvent } from '@/lib/playback';
...
type StageActionEvent =
  | { type: 'speech'; writeElementId?: string }
  | { type: 'highlight' }
  | { type: 'pause' }
  | ({ type: 'mocap' } & MocapEvent)
  | { type: string };
...
export function Stage({
  ...
  onActionChange?: (action: StageActionEvent | null) => void;
  leftPanel?: ReactNode;
}) {
  const { t } = useI18n();
  ...
  const currentSceneActionsKey = useMemo(() => {
    if (!currentScene?.actions) return '';
    return currentScene.actions.map((action) => `${action.id}:${action.type}`).join('|');
  ...
  useEffect(() => {
    ...
    const engine = new PlaybackEngine([currentScene], actionEngine, audioPlayerRef.current, {
      ...
      onSpeechStart: (text, speechAction) => {
        console.log('[Stage] onSpeechStart', {
          text,
          currentSceneId,
        });
        ...
        onActionChange?.({
          type: 'speech',
          ...(speechAction?.writeElementId ? { writeElementId: speechAction.writeElementId } : {}),
        });
      ...
      onEffectFire: (effect: Effect) => {
        if (effect.kind === 'spotlight' || effect.kind === 'laser') {
          console.log('[Stage] onEffectFire -> highlight', {
            kind: effect.kind,
            targetId: effect.targetId,
            currentSceneId,
          });

          onActionChange?.({ type: 'highlight' });
        }
      ...
      onMocapAction: (mocap) => {
        console.log('[Stage] onMocapAction', {
          mocapId: mocap.mocapId,
          mocapAction: mocap.mocapAction,
          teacherStatus: mocap.teacherStatus,
          ueAction: mocap.ueAction,
          currentSceneId,
        });

        onActionChange?.({ type: 'mocap', ...mocap });
      },
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- Rebuild only when scene identity/action sequence changes; slide content updates during playback must not reset the engine.
  }, [currentSceneId, currentSceneActionsKey]);
  ...
  const handlePlayPause = useCallback(async () => {
    ...
    console.log('[Stage] handlePlayPause', {
      mode,
      currentSceneId,
    });
    ...
    if (mode === 'playing' || mode === 'live') {
      ...
      console.log('[Stage] -> start/continue');
  ...
  }, [playbackCompleted, currentScene, currentSceneId, onActionChange]);
  ...
  const currentOutline = useStageStore((s) =>
    currentScene ? s.outlines.find((outline) => outline.order === currentScene.order) : undefined,
  );
  const isRegeneratingCurrentScene =
    !!currentOutline && generatingOutlines.some((outline) => outline.id === currentOutline.id);
  const handleRegenerateCurrentScene = useCallback(async () => {
    if (!onRetryOutline || !currentOutline || isRegeneratingCurrentScene) return;
    await onRetryOutline(currentOutline.id);
  }, [currentOutline, isRegeneratingCurrentScene, onRetryOutline]);
  ...
  const sceneViewerHeight = (() => {
    ...
    const roundtableHeight = mode === 'playback' && !isPresenting ? 132 : 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
        ...
        >
          <div className={cn('h-full min-h-0', leftPanel && !isPresenting && 'flex gap-3')}>
            {leftPanel && !isPresenting && (
              <div className="w-[460px] xl:w-[500px] 2xl:w-[540px] shrink-0 min-h-0 ml-2 xl:ml-3">
                {leftPanel}
              </div>
            )}
            <div className="flex-1 min-w-0 min-h-0 overflow-hidden rounded-2xl">
              <CanvasArea
                ...
                onRegenerateScene={
                  onRetryOutline && currentOutline ? handleRegenerateCurrentScene : undefined
                }
                regeneratingScene={isRegeneratingCurrentScene}
