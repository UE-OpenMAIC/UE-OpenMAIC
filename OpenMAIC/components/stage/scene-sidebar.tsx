...
export function SceneSidebar({
  ...
  const { scenes, currentSceneId, setCurrentSceneId, generatingOutlines, generationStatus, outlines } =
    useStageStore();
      ...
      <div className={cn('flex flex-col w-full h-full overflow-hidden', collapsed && 'hidden')}>
        ...
        >
          {scenes.map((scene, index) => {
            ...
            const outline = outlines.find((item) => item.order === scene.order);
            const isRetrying =
              !!outline &&
              (retryingOutlineId === outline.id ||
                generatingOutlines.some((item) => item.id === outline.id));
            const retryDisabled = !outline || !!retryingOutlineId || generationStatus === 'generating';
                ...
                <div className="flex justify-between items-center px-2 pt-0.5">
                  ...
                  {onRetryOutline && outline && (
                    <button
                      ...
                      className={cn(
                        'ml-1 w-6 h-6 shrink-0 rounded-md flex items-center justify-center text-gray-400 transition-all duration-150',
                      ...
                      title={isRetrying ? t('generation.regeneratingScene') : t('generation.regenerateScene')}
                    >
