...
def true_labels_from_row(cps_true, labels, n: int) -> np.ndarray:
    labels = safe_int_array(labels)
    if len(labels) == n:
        return reorder_labels(labels)
    try:
        from src.utils import create_state_labels
        return reorder_labels(np.asarray(create_state_labels(sanitize_cps(cps_true, n), labels, n), dtype=int))
    except Exception:
        return labels_from_cps(cps_true, labels, n)
...
def normalize_dataset_name(name: str) -> str:
    ...
    return aliases.get(key.lower().replace("_", "-"), key)


def load_dataset_from_clap_repo(clap_repo: Path, dataset_name: str):
    add_clap_repo(clap_repo)
    dataset_name = normalize_dataset_name(dataset_name)
    from src import utils as clap_utils
    if dataset_name == "TSSB":
        df = clap_utils.load_tssb_datasets()
    elif dataset_name == "HAS":
        df = clap_utils.load_has_datasets()
    else:
        df = clap_utils.load_datasets(dataset_name)
    return dataset_name, df


def find_segmentation_file(clap_repo: Path, dataset_name: str, configured: str | None) -> Path | None:
    if configured and str(configured).strip() and str(configured).strip().lower() not in {"auto", "none"}:
        p = Path(str(configured).strip())
    ...
    if not seg_dir.exists():
        return None
    keys = [dataset_name, dataset_name.replace("-", ""), dataset_name.replace("-", "_")]
    candidates = []
    for ext in ("*.csv.gz", "*.csv"):
        candidates.extend(seg_dir.glob(ext))
    for key in keys:
        exacts = [seg_dir / f"{key}_ClaSP.csv.gz", seg_dir / f"{key}_ClaSP.csv"]
        for p in exacts:
            if p.exists():
                return p.resolve()
    ...
    for p in candidates:
        name = p.name.lower()
        if "clasp" in name and any(k in name for k in lower_keys):
            return p.resolve()
    return None
...
def load_segmentation_df(seg_path: Path | None) -> pd.DataFrame | None:
    ...
    if "dataset" not in df.columns or "found_cps" not in df.columns:
        raise ValueError(f"Segmentation file must contain dataset and found_cps columns: {seg_path}")
    return df[["dataset", "found_cps"]]


def compute_clasp_cps(ts: np.ndarray, n_jobs: int) -> np.ndarray:
    from claspy.segmentation import BinaryClaSPSegmentation
    ts_in = np.asarray(ts, dtype=float)
    clasp = BinaryClaSPSegmentation(n_jobs=int(n_jobs))
    cps = clasp.fit_predict(ts_in)
    return sanitize_cps(cps, len(ts_in))


def get_initial_cps(dataset: str, ts: np.ndarray, seg_df: pd.DataFrame | None, settings: dict[str, str], n_jobs: int) -> tuple[np.ndarray, str]:
    ts_len = int(ts.shape[0])
    if seg_df is not None:
        hit = seg_df.loc[seg_df["dataset"].astype(str) == str(dataset)]
        if not hit.empty:
            return sanitize_cps(hit.iloc[0].found_cps, ts_len), "official_segmentation"
    if parse_bool(settings.get("fallback_compute_clasp", "1")):
        try:
            cps = compute_clasp_cps(ts, n_jobs=n_jobs)
            if len(cps) > 0:
                return cps, "computed_clasp"
        except Exception as exc:
            print(f"  WARN: computed ClaSP failed for {dataset}: {exc!r}", flush=True)
    nseg = _int_setting(settings, "fallback_uniform_segments", 8)
    cps = np.linspace(0, ts_len, nseg + 1, dtype=int)[1:-1]
    cps = sanitize_cps(cps, ts_len)
    return cps, "uniform_fallback"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run configurable CLaP benchmark PID-Meta 9x3.")
    p.add_argument("--config", type=Path, default=THIS_DIR / default_config_name())
...
def main() -> int:
    ...
    dataset_name = normalize_dataset_name(settings.get("dataset_name", "TSSB"))
    out_dir = expand_path_value(settings.get("out_dir"), default=config_dir / f"results_{dataset_name.lower().replace('-', '')}_clap_pid_meta_9x3_cgain", config_dir=config_dir, repo_root=repo_root)
    max_cases = _int_setting(settings, "max_cases", 0)
    n_jobs = _int_setting(settings, "n_jobs", 1)
    top_k = _int_setting(settings, "select_top_k_branches", 3)
    seed = _int_setting(settings, "seed", 1379)
    save_predictions = parse_bool(settings.get("save_predictions", "1"))

    print("============================================================")
    print(f"{dataset_name} CLaP configurable 9x3 PID-Meta")
    ...
    add_clap_repo(clap_repo)
    dataset_name, df_data = load_dataset_from_clap_repo(clap_repo, dataset_name)
    seg_path = find_segmentation_file(clap_repo, dataset_name, settings.get("segmentation_file", "auto"))
    seg_df = load_segmentation_df(seg_path)
    print("Dataset rows:", len(df_data))
    print("Seg file    :", seg_path if seg_path is not None else "not found; will compute ClaSP/fallback")
    ...
    case_fields = [
        "dataset", "n", "true_cps_count", "init_cps_count", "init_source", "pred_cps_count", "true_states", "pred_states",
    ...
    with case_csv.open("w", newline="", encoding="utf-8") as cf, branch_csv.open("w", newline="", encoding="utf-8") as bf, k_csv.open("w", newline="", encoding="utf-8") as kf:
        ...
        for idx, (_, row) in enumerate(df_data.iterrows(), start=1):
            ...
            status = "ok"; error = ""
            try:
                init_cps, init_source = get_initial_cps(dataset, ts, seg_df, settings, n_jobs)
                y_true = true_labels_from_row(cps_true, labels, ts_len)
                ...
                y_true = true_labels_from_row(cps_true, labels, ts_len) if len(labels) else np.zeros(ts_len, dtype=int)
            ...
            case_out = {
                ...
                "true_states": int(len(np.unique(y_true))) if len(y_true) else 0,
    ...
    summary = {
        ...
        "segmentation_file": str(seg_path) if seg_path is not None else None,
        "out_dir": str(out_dir),
