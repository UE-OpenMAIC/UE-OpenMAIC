...
title UTSA official CLaP baseline

set "SCRIPT_DIR=%~dp0"
for %%I in ("%SCRIPT_DIR%..\..") do set "REPO_ROOT=%%~fI"
set "CLAP_REPO=%REPO_ROOT%\classification-label-profile-main"
set "ENTRY=%SCRIPT_DIR%run_utsa2_clap_official_baseline.py"
set "OUT_DIR=%SCRIPT_DIR%results_utsa2_clap_official_default"
...
echo ============================================================
echo Running official CLaP default baseline on UTSA
...
call "%CONDA_CMD%" run --no-capture-output -n "%CONDA_ENV%" python -u "%ENTRY%" --repo-root "%REPO_ROOT%" --clap-repo "%CLAP_REPO%" --dataset-name "UTSA" --out-dir "%OUT_DIR%" --max-cases %MAX_CASES% --n-jobs 1 --save-predictions --compute-clasp-if-missing
