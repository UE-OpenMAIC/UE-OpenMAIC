...
r"""
PID ablation runner for your multi-branch T2S method.

Fixed version:
    The old version used text.find("[branches]"), so it could accidentally split
    at a comment line like:
    This version only treats a line whose stripped content is exactly "[branches]"
    as the real branch-table marker.

Recommended location:
    D:\code\teacherT2S\our\mocap\run_pid_ablation_mocap.py

Typical usage:
    cd /d D:\code\teacherT2S\our\mocap
    RUN_PID_ABLATION_MOCAP.cmd core quick
    RUN_PID_ABLATION_MOCAP.cmd core
    RUN_PID_ABLATION_MOCAP.cmd all
"""
...
DEFAULT_BASE_CONFIG = Path(r"D:\code\teacherT2S\our\mocap\mocap_config.txt")
DEFAULT_ENTRY = Path(r"D:\code\teacherT2S\our\mocap\run_our_mocap.py")
...
def make_variant_config(base_text: str, variant: AblationVariant, out_dir: Path, max_series: int | None, only_case_ids: str, skip_completed: bool) -> str:
    ...
    header = (
        ...
        f"# Generated automatically. Only PID/selection/voting/out_dir keys are changed.\n\n"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run PID ablation configs for multi-branch T2S.")
    ...
    for variant in variants:
        ...
        config_path = config_dir / f"mocap_pid_ablation_{variant.name}.txt"
