...
title MoCap MultiT2S controlled run


set "MOCAP_DIR=%~dp0"
for %%I in ("%MOCAP_DIR%..\..") do set "REPO_ROOT=%%~fI"
...
set "RUN_ENTRY=%MOCAP_DIR%run_our_mocap.py"
set "CONFIG_TXT=%MOCAP_DIR%mocap_config.txt"
set "OUT_DIR=%MOCAP_DIR%results_mocap_our"
...
echo ============================================================
echo MoCap MultiT2S controlled run
echo Dataset   : MoCap
...
if not exist "%RUN_ENTRY%" (
    echo [ERROR] Missing run_our_mocap.py:
    echo %RUN_ENTRY%
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo [ERROR] Dry run failed. Check paths in mocap_config.txt.
    pause
    exit /b 1
)
...
echo.
echo ============================================================
echo [4/4] Starting real MoCap run
echo ============================================================
echo.
...
if errorlevel 1 (
    echo [ERROR] MoCap run failed. Check the traceback above.
    pause
    exit /b 1
)
