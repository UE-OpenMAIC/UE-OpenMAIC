...
def load_pamap2(repo_root: Path, runtime, max_count: int | None, args: argparse.Namespace | None = None) -> list[SeriesCase]:
    ...
    Default behavior, used by all existing experiments:
        ...
        - FULL-FIT VARIANT: fit on concatenated subject101..subject108 via SeriesCase.fit_data
    ...
    This full-fit runner is a separate file to avoid affecting completed experiments.
    It uses concatenated subject101..subject108 as fit_data for PAMAP2.
    """
    ...
    for i in range(1, 9):
        ...
        loaded.append((i, data, labels))
        fit_parts.append(data)

    if not loaded:
        raise FileNotFoundError(f"No strict PAMAP2 subject10*.dat cases found under {protocol_dir}")

    train_data = runtime["np"].vstack(fit_parts)
    print(
        f"[PAMAP2] full_fit=1 fit_subjects={len(fit_parts)} fit_rows={int(train_data.shape[0])} "
        f"fit_dims={int(train_data.shape[1])}",
        flush=True,
    )

    cases = []
    for i, data, labels in loaded:
...
def run_multit2s_case(case: SeriesCase, args, runtime):
    ...
    return {
        ...
        "state_info": best["state_info"],
        "k_scores": scored,
        "meta_seq": meta_seq,
        "meta_ratio_df": best["ratio_df"],
        "ARI": ari,
        "NMI": nmi,
        "K": best["K"],
...
CASE_RESULT_FIELDS = ["dataset", "case_id", "rows", "candidate_branches_ran", "meta_branches_used", "branches_ran", "selected_K", "n_pred_states", "segments", "meta_min_len", "branch_select_metric", "meta_vote_weight_mode", "selected_branch_names", "ARI", "NMI", "seconds"]
