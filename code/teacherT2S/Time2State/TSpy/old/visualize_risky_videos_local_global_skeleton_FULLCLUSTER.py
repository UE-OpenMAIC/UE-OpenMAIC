...
LAYER2_DIR_DEFAULT = T2S_ROOT_DEFAULT / "_cross_video_prototype_alignment_X1_only"

LAYER2_SEGMENT_CSV_DEFAULT = LAYER2_DIR_DEFAULT / "layer2_cross_video_prototype_aligned_segments_X1_only.csv"

LAYER2_MANIFEST_DEFAULT = LAYER2_DIR_DEFAULT / "prototype_alignment_X1_only_build_manifest.csv"

OUT_DIR_DEFAULT = LAYER2_DIR_DEFAULT / "_viz_risky_videos_local_global_skeleton_fullcluster"
...
RISKY_VIDEO_IDS_DEFAULT = "4,21,28,36,41,47,50,34,35,51"
...
def auto_find_layer2_outputs(t2s_root: Path):
    ...
    for p in t2s_root.rglob("layer2_cross_video_prototype_aligned_segments_X1_only.csv"):
        ...
        if parent_name == "_cross_video_prototype_alignment_X1_only":

            score += 300

        if "anchor" not in parent_name.lower():

            score += 80

        if "anchor" in parent_name.lower():
...
def main():
    ...
    ap.add_argument("--video-ids", type=str, default=RISKY_VIDEO_IDS_DEFAULT, help="逗号分隔；默认优先处理风险视频；若想全部视频，传入 --video-ids \"\"")
    ...
    if not layer2_segment_csv.exists():
        ...
        if found_segment is not None and found_segment.exists():
            ...
            if str(args.out_dir) == str(OUT_DIR_DEFAULT):
                ...
                out_dir = found_dir / "_viz_risky_videos_local_global_skeleton_fullcluster"
    ...
    if args.video_ids.strip() == "":
        ...
        run_video_ids = all_video_ids

    print("=" * 100)

    print("Visualize RISKY videos: local state + global state + skeleton - FULL CLUSTER, NO POSTERIOR")
    ...
    print(f"default_risky_ids = {RISKY_VIDEO_IDS_DEFAULT}")
    ...
    for i, video_id in enumerate(run_video_ids, start=1):
        ...
        role = role_map.get(norm_video_id(video_id), "full_recluster")
