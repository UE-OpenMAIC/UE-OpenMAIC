...
if /I not "%~1"=="__inner__" (
    start "TICC MoCap strict baseline - DIRECT PRINT" cmd /k ""%~f0" __inner__"
    exit /b 0
)
set "SCRIPT_DIR=%~dp0"
for %%I in ("%SCRIPT_DIR%..") do set "TICC_ROOT=%%~fI"
set "SHARD_DIR=%TICC_ROOT%\_shard"
set "CONFIG=%SCRIPT_DIR%mocap_ticc_config.txt"
set "CONDA_ENV=multit2s_cuda"
echo ============================================================
echo TICC MoCap strict baseline - DIRECT PRINT
