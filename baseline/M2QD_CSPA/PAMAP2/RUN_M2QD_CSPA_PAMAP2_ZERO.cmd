...
title M2QD_CSPA PAMAP2_zero run
set "DATASET_DIR=%~dp0"
for %%I in ("%DATASET_DIR%..\..\..") do set "REPO_ROOT=%%~fI"
set "M2QD_CSPA_DIR=%REPO_ROOT%\baseline\M2QD_CSPA"
set "SHARED_DIR=%M2QD_CSPA_DIR%\_shared"
set "BENCH_DIR=%REPO_ROOT%\multi_t2s_paper_benchmark"
set "ENV_FILE=%BENCH_DIR%\environment_cuda.yml"
set "RUN_ENTRY=%DATASET_DIR%run_m2qd_cspa_PAMAP2_zero.py"
set "CONFIG_TXT=%DATASET_DIR%pamap2_zero_config.txt"
set "OUT_DIR=%DATASET_DIR%results_m2qd_cspa_pamap2_zero"
...
if /I "%~1"=="--dry-run" (
    ...
    echo M2QD_CSPA_DIR=%M2QD_CSPA_DIR%
...
echo ============================================================
echo M2QD_CSPA baseline run
echo Dataset   : PAMAP2_zero
...
if not exist "%SHARED_DIR%\m2qd_cspa_multit2s_runner.py" (
    echo [ERROR] Missing m2qd_cspa_multit2s_runner.py:
    echo %SHARED_DIR%\m2qd_cspa_multit2s_runner.py
    pause
    exit /b 1
)
...
echo.
echo ============================================================
echo [4/4] Starting real PAMAP2_zero M2QD_CSPA run
echo ============================================================
echo.
call "%CONDA_CMD%" run --no-capture-output -n "%CONDA_ENV%" python -u "%RUN_ENTRY%" --config "%CONFIG_TXT%"
if errorlevel 1 (
    echo [ERROR] M2QD_CSPA run failed. Check the traceback above.
    pause
    exit /b 1
)
echo.
