...
chcp 65001 >nul
title T2S baseline ActRecTut - no flash logger
cd /d "%~dp0"
set "METHOD_DIR=%~dp0"
for %%I in ("%METHOD_DIR%..") do set "T2S_ROOT=%%~fI"
for %%I in ("%T2S_ROOT%..\..") do set "REPO_ROOT=%%~fI"
set "RUN_ENTRY=%METHOD_DIR%run_t2s_actrectut.py"
set "CONFIG_TXT=%METHOD_DIR%actrectut_config.txt"
set "OUT_DIR=%METHOD_DIR%results_t2s_actrectut_paper_grid"
...
set "T1=%T1: =0%"
set "LOG_FILE=%LOG_DIR%\t2s_actrectut_%D1%_%D2%_%D3%_%T1%_%T2%_%T3%.log"
cls
echo ============================================================
echo T2S baseline ActRecTut - no flash logger
...
if not exist "%RUN_ENTRY%" (
    echo [ERROR] Missing run_t2s_actrectut.py:
    echo %RUN_ENTRY%
    echo [ERROR] Missing run_t2s_actrectut.py: %RUN_ENTRY%>>"%LOG_FILE%"
    goto FAIL
)
if not exist "%CONFIG_TXT%" (
    echo [ERROR] Missing actrectut_config.txt:
    echo %CONFIG_TXT%
    echo [ERROR] Missing actrectut_config.txt: %CONFIG_TXT%>>"%LOG_FILE%"
    goto FAIL
)
...
echo.
echo ============================================================
echo [3/3] Starting real T2S ActRecTut run
echo ============================================================
echo.
echo ============================================================>>"%LOG_FILE%"
echo [3/3] Starting real T2S ActRecTut run>>"%LOG_FILE%"
echo ============================================================>>"%LOG_FILE%"
...
if errorlevel 1 (
    echo [ERROR] T2S ActRecTut run failed. Check the traceback above.
    echo [ERROR] T2S ActRecTut run failed.>>"%LOG_FILE%"
    goto FAIL
)
echo.
echo ============================================================
