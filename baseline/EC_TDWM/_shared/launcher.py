...
EC_TDWM_ROOT = THIS_DIR.parent
BASELINE_ROOT = EC_TDWM_ROOT.parent
REPO_ROOT = BASELINE_ROOT.parent

OUR_ROOT = EC_TDWM_ROOT
BENCH_DIR = REPO_ROOT / "multi_t2s_paper_benchmark"
DEFAULT_RUNNER = THIS_DIR / "ec_tdwm_multit2s_runner.py"
...
def expand_path_value(value: str | None, *, default: Path, config_dir: Path, script_dir: Path, repo_root: Path) -> Path:
    ...
    text = text.replace("{EC_TDWM_ROOT}", str(EC_TDWM_ROOT))
    text = text.replace("{EC_TDWM_ROOT}", str(EC_TDWM_ROOT))
...
def build_command(config_path: Path, dataset_default: str) -> tuple[list[str], Path, Path]:
    ...
    for key, flag in [
        ...
        ("ec_tdwm_k_mode", "--ec-tdwm-k-mode"),
        ("ec_tdwm_engine", "--ec-tdwm-engine"),
        ("ec_tdwm_max_iter", "--ec-tdwm-max-iter"),
        ("ec_tdwm_tol", "--ec-tdwm-tol"),
        ("ec_tdwm_agglomerative_max_rows", "--ec-tdwm-agglomerative-max-rows"),
        ("ec_tdwm_batch_size", "--ec-tdwm-batch-size"),
    ]:
    ...
    if parse_bool(settings.get("ec_tdwm_apply_meta_min_len", "0")):
        cmd.append("--ec-tdwm-apply-meta-min-len")
