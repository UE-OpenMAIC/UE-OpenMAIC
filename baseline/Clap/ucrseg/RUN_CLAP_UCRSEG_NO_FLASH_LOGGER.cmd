...
chcp 65001 >nul
title CLaP baseline UCR-SEG - no flash logger
cd /d "%~dp0"
set "METHOD_DIR=%~dp0"
for %%I in ("%METHOD_DIR%..") do set "CLAP_ROOT=%%~fI"
for %%I in ("%CLAP_ROOT%..\..") do set "REPO_ROOT=%%~fI"
set "RUN_ENTRY=%METHOD_DIR%run_clap_ucrseg.py"
set "CONFIG_TXT=%METHOD_DIR%ucrseg_config.txt"
set "OUT_DIR=%METHOD_DIR%results_ucrseg_clap_baseline_default"
...
set "T1=%T1: =0%"
set "LOG_FILE=%LOG_DIR%\clap_ucrseg_%D1%_%D2%_%D3%_%T1%_%T2%_%T3%.log"
cls
echo ============================================================
echo CLaP baseline UCR-SEG - no flash logger
...
echo.
echo ============================================================
echo [3/3] Starting real CLaP UCR-SEG run
echo ============================================================
echo.
echo ============================================================>>"%LOG_FILE%"
echo [3/3] Starting real CLaP UCR-SEG run>>"%LOG_FILE%"
echo ============================================================>>"%LOG_FILE%"
...
if errorlevel 1 (
    echo [ERROR] CLaP UCR-SEG run failed. Check the traceback above.
    echo [ERROR] CLaP UCR-SEG run failed.>>"%LOG_FILE%"
    goto FAIL
)
echo.
echo ============================================================
