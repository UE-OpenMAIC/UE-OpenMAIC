...
chcp 65001 >nul
title CLaP baseline Synthetic - no flash logger
cd /d "%~dp0"
set "METHOD_DIR=%~dp0"
for %%I in ("%METHOD_DIR%..") do set "CLAP_ROOT=%%~fI"
for %%I in ("%CLAP_ROOT%..\..") do set "REPO_ROOT=%%~fI"
set "RUN_ENTRY=%METHOD_DIR%run_clap_synthetic.py"
set "CONFIG_TXT=%METHOD_DIR%synthetic_config.txt"
set "OUT_DIR=%METHOD_DIR%results_synthetic_clap_baseline_default"
...
set "T1=%T1: =0%"
set "LOG_FILE=%LOG_DIR%\clap_synthetic_%D1%_%D2%_%D3%_%T1%_%T2%_%T3%.log"
cls
echo ============================================================
echo CLaP baseline Synthetic - no flash logger
...
echo.
echo ============================================================
echo [3/3] Starting real CLaP Synthetic run
echo ============================================================
echo.
echo ============================================================>>"%LOG_FILE%"
echo [3/3] Starting real CLaP Synthetic run>>"%LOG_FILE%"
echo ============================================================>>"%LOG_FILE%"
...
if errorlevel 1 (
    echo [ERROR] CLaP Synthetic run failed. Check the traceback above.
    echo [ERROR] CLaP Synthetic run failed.>>"%LOG_FILE%"
    goto FAIL
)
echo.
echo ============================================================
